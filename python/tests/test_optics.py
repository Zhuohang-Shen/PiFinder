...
def test_display_to_solve_maps_the_same_sensor_pixel(name, rotation):
    ...
    scale = max(profile.raw_size) / DISPLAY_FRAME_SIZE / profile.solve_bin_factor
...
def test_display_fov_recovers_the_known_crop_field():
    ...
    assert geometry.solve_width == 1080 // profile.solve_bin_factor
    ...
    crop_width_binned = 980 / profile.solve_bin_factor
    expected = math.degrees(2 * math.atan((crop_width_binned / 2) / focal_px))
...
def test_full_frame_covers_more_sky_than_the_crop(name):
    ...
    solve_area = np.prod(profile.solve_frame_size) * profile.solve_bin_factor**2
...
@pytest.mark.unit
def test_bayer_profiles_bin_and_mono_profiles_do_not():
    assert CAMERA_PROFILES["imx462"].solve_frame_size == (960, 540)
    assert CAMERA_PROFILES["hq"].solve_frame_size == (1014, 760)
    assert CAMERA_PROFILES["imx296"].solve_frame_size == (1456, 1088)


@pytest.mark.unit
def test_bayer_binning_averages_each_mosaic_quad():
    profile = CAMERA_PROFILES["imx462"]
    width, height = profile.raw_size
    raw = np.zeros((height, width), dtype=np.uint16)
    # One RGGB quad with a known mean.
    raw[0, 0], raw[0, 1], raw[1, 0], raw[1, 1] = 10, 20, 30, 40

    binned = profile.full_frame(raw)

    assert binned.shape == (height // 2, width // 2)
    assert binned[0, 0] == pytest.approx(25.0)
